...
class MpesaPaymentAdmin(admin.ModelAdmin):
    ...
    def export_to_pdf(self, modeladmin, request, queryset):
     ...
     # Add heading
        heading = Paragraph("<b>Payments Report</b>", getSampleStyleSheet()['Heading1'])
     ...
     # Add space before the table
        elements.append(Paragraph("<br/><br/>", getSampleStyleSheet()['Normal']))
         ...
         # table data
        data = [['#', 'User', 'Amount', 'Date Paid']]
        for index, payment in enumerate(queryset, start=1):
            # Format the date to include only the date portion
            formatted_date = payment.date.strftime("%Y-%m-%d")
            data.append([index, payment.user, payment.amount, formatted_date])
