...
import os
from recruiters.forms import RecruiterForm
...
class InternshipAdmin(admin.ModelAdmin):
   #  form = RecruiterForm
    list_display = ['id', 'title', 'recruiter_id', 'recruiter']
    list_filter = ('recruiter', 'post_date') 
    ...
    def export_to_pdf(self, modeladmin, request, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename="internships.pdf"'
    ...
    # Create a PDF document
     ...
     # Add heading
        heading = Paragraph("<b>Internships Report</b>", getSampleStyleSheet()['Heading1'])
     ...
     # table data
        data = [['#','Recruiter', 'Title', 'Post_date']]
        for index, internship in enumerate(queryset, start=1):
         data.append([index, internship.recruiter, internship.title, internship.post_date])
